...
if __name__ == "__main__":
    ...
    status = lines.map(extractStatusRequest)

    # Reduce by Status  over a 5-minute window sliding every second
    statusCounts = status.map(lambda x: (x, 1)).reduceByKeyAndWindow(lambda x, y: x + y, lambda x, y : x - y, 300, 5)
